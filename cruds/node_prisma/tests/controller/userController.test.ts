...
describe('User Controller Tests', () => {
    ...
    test('createUser should create a new user', async () => {
        ...
        expect(createUserService).toHaveBeenCalledWith('John Doe', 'john@example.com', 'securepassword');
    ...
    test('getUserById should return a user', async () => {
        ...
        (getByIdService as jest.Mock).mockResolvedValue(user);

        await getUserById(req as Request, res as Response, next);

        expect(getByIdService).toHaveBeenCalledWith(1);
        expect(res.status).toHaveBeenCalledWith(StatusCodes.OK);
    ...
    test('getUserByEmail should return a user', async () => {
        ...
        const user = { id: 1, name: 'John Doe', email: 'john@example.com' };
        (getByEmailService as jest.Mock).mockResolvedValue(user);

        await getUserByEmail(req as Request, res as Response, next);

        expect(getByEmailService).toHaveBeenCalledWith('john@example.com');
        expect(res.status).toHaveBeenCalledWith(StatusCodes.OK);
        expect(res.send).toHaveBeenCalledWith(user);
    });
    ...
    test('updateUser should update a user', async () => {
        ...
        const userUpdated = { id: 1, name: 'John Updated', email: 'john@example.com' };
        (updateUserService as jest.Mock).mockResolvedValue(userUpdated);

        await updateUser(req as Request, res as Response, next);

        expect(updateUserService).toHaveBeenCalledWith(1, 'John Updated', 'newpassword');
        expect(res.status).toHaveBeenCalledWith(StatusCodes.OK);
    ...
    test('loginController should return a token for valid credentials', async () => {
        ...
        const loginResult = { rest: { id: 1, name: 'John Doe', email: 'john@example.com' }, token: 'fake-token' };
        (loginService as jest.Mock).mockResolvedValue(loginResult);

        await loginController(req as Request, res as Response, next);

        expect(loginService).toHaveBeenCalledWith('john@example.com', 'securepassword');
        expect(res.status).toHaveBeenCalledWith(StatusCodes.OK);
